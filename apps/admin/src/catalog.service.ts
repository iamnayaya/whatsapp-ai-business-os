...
export class CatalogService {
  ...
  private async importOne(item: CatalogImportItem, index: number): Promise<CatalogImportResult> {
    const loggerCtx = { filename: item.filename };
    ...
    if (!generated.usable || !generated.listing) {
      ...
      this.deps.logger.warn('catalog image rejected', { ...loggerCtx, reason: generated.reason });
      return { filename: item.filename, ok: false, status: 'image_rejected', reason: generated.reason };
    }

    // Human review gate — never auto-publish.
    const decision = await this.deps.reviewer({ filename: item.filename, listing: generated.listing });
    ...
    if (decision.action === 'reject') {
      ...
      return { filename: item.filename, ok: false, status: 'review_rejected', listing: generated.listing };
    }

    const listing = decision.action === 'edit' ? this.applyEdit(generated.listing, decision) : generated.listing;
    const sku = item.sku ?? `AUTO-${Date.now().toString(36).toUpperCase()}-${index + COUNTER_OFFSET}`;
    ...
    await this.deps.audit.record({
      ...
      entityType: 'PRODUCT',
      entityId: product.id,
      details: { filename: item.filename, sku, title: listing.title, category: listing.category },
    });
    this.deps.logger.info('catalog item published', { ...loggerCtx, sku, productId: product.id });

    return { filename: item.filename, ok: true, status: 'generated', listing, productId: product.id, sku };
  }

  private applyEdit(base: GeneratedListing, decision: Extract<ReviewDecision, { action: 'edit' }>): GeneratedListing {
    return {
